...
public class SwerveModuleIOTalonFXPhoenix6 implements SwerveModuleIO {
  ...
  private Double anglePositionStatusSignal;
  private Double angleVelocityStatusSignal;
  private Double anglePositionErrorStatusSignal;
  private Double anglePositionReferenceStatusSignal;
  private Double drivePositionStatusSignal;
  private Double driveVelocityStatusSignal;
  private Double driveVelocityErrorStatusSignal;
  private Double driveVelocityReferenceStatusSignal;
  ...
  public SwerveModuleIOTalonFXPhoenix6(
    ...
    if (RobotConfig.getInstance().getSwerveType() == SwerveConstants.SwerveType.MK4) {
      ...
      wheelCircumference = SwerveConstants.MK4I_L2_WHEEL_CIRCUMFERENCE;
      driveGearRatio = SwerveConstants.MK4I_L2_DRIVE_GEAR_RATIO;
      driveMotorInverted = SwerveConstants.MK4I_L2_DRIVE_MOTOR_INVERTED;
      angleGearRatio = SwerveConstants.MK4I_L2_ANGLE_GEAR_RATIO;
      angleMotorInverted = SwerveConstants.MK4I_L2_ANGLE_MOTOR_INVERTED;
      canCoderInverted = SwerveConstants.MK4I_L2_CAN_CODER_INVERTED;
    }
  ...
  private void configAngleEncoder(int canCoderID) {
    ...
    for (int i = 0; i < 5; ++i) {
      status = ((CoreCANcoder) this.angleEncoder).getConfigurator().apply(config);
      if (status.isOK()) {

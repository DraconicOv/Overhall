...
public class DriveToPose extends Command {
  ...
  public void execute() {
    ...
    if (driveKp.hasChanged()
        ...
        || thetaTolerance.hasChanged()) { // If anything has changed
      xController.setP(driveKp.get()); // set the new values
    ...
    drivetrain.drive(xVelocity, yVelocity, thetaVelocity, true, true); // Drive
  }
  ...
  public boolean isFinished() {
    ...
    return !drivetrain.isMoveToPoseEnabled()
        || this.timer.hasElapsed(timeout.get()) // Make sure we haven't timed out

...
public class Vision extends SubsystemBase {
  ...
  private final VisionLogger[] ios;
  ...
  public Vision(VisionIO[] visionIOs) {
    ...
    this.ios = new VisionLogger[visionIOs.length];
    for (int i = 0; i < visionIOs.length; i++) {
      this.ios[i] = new VisionLogger();
    }
    ...
    for (AprilTag tag : layout.getTags()) {
      Logger.recordOutput(VisionConstants.SUBSYSTEM_NAME + "/AprilTags/" + tag.ID, tag.pose);
    }
  }
  ...
  public void periodic() {
    ...
    for (int i = 0; i < visionIOs.length; i++) {
      ...
      Logger.processInputs(VisionConstants.SUBSYSTEM_NAME + "/" + i, ios[i]);
  ...
  public boolean posesHaveConverged() {
    for (int i = 0; i < visionIOs.length; i++) {
      ...
      if (odometry.getEstimatedPosition().minus(robotPose.toPose2d()).getTranslation().getNorm()
        ...
        Logger.recordOutput(VisionConstants.SUBSYSTEM_NAME + "/posesInLine", true);
        return true;
      }
    }
    Logger.recordOutput(VisionConstants.SUBSYSTEM_NAME + "/posesInLine", false);
    return false;
  }
  ...
  private Matrix<N3, N1> getStandardDeviations(int index, Pose2d estimatedPose) {
    ...
    if (numTags == 1 && avgDist > VisionConstants.MAX_DISTANCE_TO_TARGET_METERS) {
